...
import { PRAISES as PRAISES_FROM_DATA } from "./data.js";
...
const PHRASES = (Array.isArray(PRAISES_FROM_DATA) && PRAISES_FROM_DATA.length)
  ? PRAISES_FROM_DATA
      .map(p => [p.t, p.s].filter(Boolean).join(" ").trim())
      .filter(Boolean)
  : [
      "Ты невероятная 💗",
